...
/**
 * HttpPutStep only overrides getType()/getMethod() - all request-building logic is inherited
 * from HttpPostStep (see HttpPostStepTest, currently non-functional, for that logic's own coverage).
 * This test focuses on what's actually specific to this class: the PUT method is genuinely used
...
class HttpPutStepTest {

    @Test
    void getType_shouldReturnHttpPut() {
        assertEquals("http.put", new HttpPutStep().getType());
    }

    @Test
    void getMethod_shouldReturnPut() {
        assertEquals(HttpMethod.PUT, new HttpPutStep().getMethod());
    }

    @Test
    void execute_shouldCallHttpHelperWithPutMethod() throws StepExecutionException, DSLExecutionException {
        ...
        HttpPutStep step = new HttpPutStep();
        step.setName("update_resource");
        step.setArgs(args);
        step.setResultName("the_response");

        step.execute(di);

        verify(httpHelper).doMethod(eq(HttpMethod.PUT), eq("http://example.com/resource/123"),
